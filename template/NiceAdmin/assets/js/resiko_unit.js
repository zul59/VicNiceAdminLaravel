(async function() {
    ...
    async function getResikoUnitList() {
        try {
            const path = 'https://nice-admin1.herokuapp.com/risk';
    ...
    function editResiko(unitName, riskId) {
        location.href = 'Tambah Data Risiko.html' + `?unit=${encodeURIComponent(unitName)}&type=edit&id=${riskId}`;
    }

    async function removeResiko(riskId) {
        try {
            const path = `https://nice-admin1.herokuapp.com/risk/${riskId}`;
            const response = await fetch(path, {
