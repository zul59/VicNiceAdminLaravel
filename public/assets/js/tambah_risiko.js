(async function () {
    ...
    if (session != null) {
        ...
        if (sessionData.role == 2) {
            // if admin
            // document.getElementById("unit").setAttribute("disabled", false);
        } else {
    ...
    const inputUnitTerkaitPenyebabRisiko = document.getElementById(
        "unitTerkaitPenyebabRisiko"
    );

    const inputRiskInherrentLikelihood = document.getElementById(
        "riskInherrentLikelihood"
    );
    const inputRiskInherrentImpact = document.getElementById(
        "riskInherrentImpact"
    );
    const inputRiskInherrentRating = document.getElementById(
        "riskInherrentRating"
    );
    const inputRiskInherrentMatrix = document.getElementById(
        "riskInherrentMatrix"
    );

    const inputRiskTreatmentAction = document.getElementById(
        "riskTreatmentAction"
    );
    const inputRiskTreatmentPlan = document.getElementById("riskTreatmentPlan");
    const inputRiskTreatmentResolveBy = document.getElementById(
        "riskTreatmentResolveBy"
    );

    const inputRiskResidualLikelihood = document.getElementById(
        "riskResidualLikelihood"
    );
    const inputRiskResidualImpact =
        document.getElementById("riskResidualImpact");
    const inputRiskResidualRating =
        document.getElementById("riskResidualRating");
    const inputRiskResidualMatrix =
        document.getElementById("riskResidualMatrix");

    const inputRiskMitigationOpsiRisiko = document.getElementById(
        "riskMitigationOpsiRisiko"
    );
    const inputRiskMitigationDeskripsiTindakanMitigasi =
        document.getElementById("riskMitigationDeskripsiTindakanMitigasi");

    const inputRiskAfterMitigationLikelihood = document.getElementById(
        "riskAfterMitigationLikelihood"
    );
    const inputRiskAfterMitigationImpact = document.getElementById(
        "riskAfterMitigationImpact"
    );
    const inputRiskAfterMitigationRating = document.getElementById(
        "riskAfterMitigationRating"
    );
    const inputRiskAfterMitigationMatrix = document.getElementById(
        "riskAfterMitigationMatrix"
    );

    const inputRiskMonitoringMethod = document.getElementById(
        "riskMonitoringMethod"
    );
    const inputRiskMonitoringProgress = document.getElementById(
        "riskMonitoringProgress"
    );
    const inputRiskMonitoringStatus = document.getElementById(
    ...
    async function init() {
        try {
            ...
            if (unit) {
                const unitSelectedIndex = units.findIndex(
                    (u) => u.unit_name === unit.trim()
                );
    ...
    function updateInherrentRatingAndMatrix() {
        const likelihood =
            inputRiskInherrentLikelihood.value === ""
                ? 1
                : inputRiskInherrentLikelihood.value;
        const impact =
            inputRiskInherrentImpact.value === ""
                ? 1
                : inputRiskInherrentImpact.value;
    ...
    function updateResidualRatingAndMatrix() {
        const likelihood =
            inputRiskResidualLikelihood.value === ""
                ? 1
                : inputRiskResidualLikelihood.value;
        const impact =
            inputRiskResidualImpact.value === ""
                ? 1
                : inputRiskResidualImpact.value;
    ...
    function updateAfterMitigationRatingAndMatrix() {
        const likelihood =
            inputRiskAfterMitigationLikelihood.value === ""
                ? 1
                : inputRiskAfterMitigationLikelihood.value;
        const impact =
            inputRiskAfterMitigationImpact.value === ""
                ? 1
                : inputRiskAfterMitigationImpact.value;
    ...
    async function tambahRisiko(ev) {
        ...
        const data = {
            ...
            riskCategory:
                riskCategorySelect.options[riskCategorySelect.selectedIndex]
                    .text,
            cause: inputCause.value,
            sourceOfRisk:
                sourceOfRiskSelect.options[sourceOfRiskSelect.selectedIndex]
                    .text,
            ...
            riskMitigationDeskripsiTindakanMitigasi:
                inputRiskMitigationDeskripsiTindakanMitigasi.value,
            riskAfterMitigationLikelihood:
                inputRiskAfterMitigationLikelihood.value,
    ...
    async function fetchExistingData(riskId) {
        try {
            ...
            const unitSelectedIndex = units.findIndex(
                (u) => u.unit_name === result.data.unit_name
            );
            if (unitSelectedIndex != -1) {
                unitSelect.value = `unit-${unitSelectedIndex}`;
            }

            const riskCategorySelectedIndex = riskCategories.findIndex(
                (u) => u.risk_category_name === result.data.risk_category
            );
            if (riskCategorySelectedIndex != -1) {
                riskCategorySelect.value = `risk-category-${riskCategorySelectedIndex}`;
            }

            const sourceOfRiskSelectedIndex = sourceOfRisks.findIndex(
                (u) => u.source_of_risk_name === result.data.source_of_risk
            );
            if (sourceOfRiskSelectedIndex != -1) {
                sourceOfRiskSelect.value = `source-of-risk-${sourceOfRiskSelectedIndex}`;
            }

            const standarSelectedIndex = standarAcademics.findIndex(
                (u) => u.standar_academic_name === result.data.standar
            );
            ...
            inputUnitTerkaitPenyebabRisiko.value =
                result.data.unit_terkait_penyebab_resiko;
            inputRiskInherrentLikelihood.value =
                result.data.risk_inherrent_likelihood;
            ...
            inputRiskTreatmentResolveBy.value =
                result.data.risk_treatment_resolve_by;
            inputRiskResidualLikelihood.value =
                result.data.residual_risk_likelihood;
            inputRiskResidualImpact.value = result.data.residual_risk_impact;
            inputRiskMitigationOpsiRisiko.value =
                result.data.risk_mitigation_opsi_risiko;
            inputRiskMitigationDeskripsiTindakanMitigasi.value =
                result.data.risk_mitigation_deskripsi_tindakan_mitigasi;
            inputRiskAfterMitigationLikelihood.value =
                result.data.risk_after_mitigation_likelihood;
            inputRiskAfterMitigationImpact.value =
                result.data.risk_after_mitigation_impact;
            inputRiskMonitoringMethod.value =
                result.data.risk_monitoring_method;
            inputRiskMonitoringProgress.value =
                result.data.risk_monitoring_progress;
            inputRiskMonitoringStatus.value =
                result.data.risk_monitoring_status;
